...
// Marcas del stack — íconos monocromáticos (fill=currentColor) que revelan su color al hover.
const TECH_ICONS = {
  react:
...
const TECHS: Tech[] = [
  { key: 'react', name: 'React', brand: '#61DAFB', x: 19, y: 26, depth: 'near' },
  { key: 'next', name: 'Next.js', brand: '#FFFFFF', x: 50, y: 12, depth: 'far' },
  { key: 'typescript', name: 'TypeScript', brand: '#3178C6', x: 81, y: 24, depth: 'near' },
  { key: 'node', name: 'Node.js', brand: '#5FA04E', x: 87, y: 60, depth: 'mid' },
  { key: 'tailwind', name: 'Tailwind CSS', brand: '#38BDF8', x: 71, y: 84, depth: 'near' },
  { key: 'docker', name: 'Docker', brand: '#2496ED', x: 50, y: 91, depth: 'mid' },
  { key: 'postgresql', name: 'PostgreSQL', brand: '#5B9BD5', x: 28, y: 84, depth: 'far' },
  { key: 'python', name: 'Python', brand: '#5A9FD4', x: 13, y: 60, depth: 'mid' },
];
...
const TECH_DEPTH: Record<
  Depth,
  { scale: number; blur: number; op: number; par: number; z: number }
> = {
  near: { scale: 1, blur: 0, op: 1, par: 0.9, z: 12 },
  mid: { scale: 0.9, blur: 0.6, op: 0.9, par: 0.5, z: 9 },
  far: { scale: 0.8, blur: 1.3, op: 0.78, par: 0.26, z: 6 },
};
...
function TechNode({ tech, i, reduced }: { tech: Tech; i: number; reduced: boolean }) {
  const d = TECH_DEPTH[tech.depth];
  const floats = ['mc-fl1', 'mc-fl2', 'mc-fl3'];
  const dur = (8 + (i % 4) * 0.9).toFixed(1);
  const anchor = {
    position: 'absolute',
    left: `${tech.x}%`,
    top: `${tech.y}%`,
  ...
  const cap: CSSProperties = {
    ...
    boxShadow: '0 10px 30px -14px rgba(0,0,0,.6)',
    color: 'rgba(231,235,243,.85)',
    transform: `scale(${d.scale})`,
    opacity: d.op,
  };
  const above = tech.y > 72;
  const lbl: CSSProperties = {
    position: 'absolute',
    left: '50%',
    ...(above
      ? { bottom: 'calc(100% + 10px)', top: 'auto', transform: 'translate(-50%,-5px)' }
      : { top: 'calc(100% + 10px)', transform: 'translate(-50%,5px)' }),
    whiteSpace: 'nowrap',
    fontSize: '.78rem',
    fontWeight: 600,
    color: 'var(--text)',
  ...
  return (
    <div className="mc-tech" tabIndex={0} role="img" aria-label={tech.name} style={anchor}>
      <div
        className="mc-par"
        data-par={d.par}
        style={{
      ...
      >
        <div
          className="mc-blur"
          data-reveal="1"
          data-reveal-delay={String(120 + i * 110)}
          style={{ position: 'absolute', inset: 0, filter: `blur(${d.blur}px)` }}
        >
          <div
            style={
              reduced
                ...
                : {
                    position: 'absolute',
                    inset: 0,
                    animation: `${floats[i % 3]} ${dur}s ease-in-out infinite ${(i * 0.35).toFixed(1)}s`,
                  }
            }
          >
            <div className="mc-cap" style={cap}>
              <span className="mc-ico" style={{ display: 'flex' }}>
                <TechIcon techKey={tech.key} size={30} />
              </span>
            </div>
          </div>
        </div>
        <span className="mc-lbl" style={lbl}>
          {tech.name}
        </span>
      </div>
    </div>
...
function TechLines({ reduced }: { reduced: boolean }) {
  const paths: ReactElement[] = [];
  for (let i = 0; i < TECH_EDGES.length; i++) {
    const a = TECHS[TECH_EDGES[i][0]];
    const b = TECHS[TECH_EDGES[i][1]];
    ...
    paths.push(
      <path
        ...
        style={{
          animation: reduced ? 'none' : `mc-lineflow ${2.6 + (i % 3) * 0.5}s linear infinite`,
          filter: 'drop-shadow(0 0 2px rgba(34,211,238,.6))',
        }}
      />,
    );
  }
...
function TechRing({
  ...
  const anim = reduced ? 'none' : `${rev ? 'mc-spin-rev' : 'mc-spin'} ${dur}s linear infinite`;
  return (
    <div
      aria-hidden="true"
      style={{
    ...
    >
      <span
        style={{
          ...
          borderRadius: '50%',
          background: dotColor,
          boxShadow: `0 0 10px 1px ${dotColor}`,
        }}
      ></span>
    </div>
  );
...
const PARTICLES: [number, number, number, string, number, number][] = [
  [12, 20, 3, 'var(--accent)', 0.5, 15],
  [86, 28, 2, 'var(--primary)', 0.45, 18],
  [22, 74, 3, 'var(--accent)', 0.4, 20],
  [78, 80, 2, 'var(--accent)', 0.45, 17],
  [50, 16, 2, 'var(--primary)', 0.4, 16],
  [8, 50, 2, 'var(--accent)', 0.4, 21],
  [92, 58, 3, 'var(--primary)', 0.35, 19],
  [40, 88, 2, 'var(--accent)', 0.4, 22],
  [64, 10, 2, 'var(--accent)', 0.35, 16],
  [33, 40, 2, 'var(--primary)', 0.3, 23],
];

function TechParticles({ reduced }: { reduced: boolean }) {
  return (
    <div
      aria-hidden="true"
      style={{
      ...
      {PARTICLES.map((p, i) => (
        <span
          ...
          style={{
            ...
            ...(reduced
              ? {}
              : { animation: `mc-drift ${p[5]}s ease-in-out infinite ${i * 0.4}s` }),
          }}
        ></span>
      ))}
    </div>
...
export default function StackViz() {
  const { isMobile, reducedMotion } = useSite();
  ...
  const onStackMove = (e: ReactMouseEvent<HTMLDivElement>) => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      const max = 22;
      const pars = stage.querySelectorAll<HTMLElement>('.mc-par');
      for (let i = 0; i < pars.length; i++) {
        const d = parseFloat(pars[i].getAttribute('data-par') || '') || 0;
        pars[i].style.transform =
          'translate(' + (nx * max * d).toFixed(1) + 'px,' + (ny * max * d).toFixed(1) + 'px)';
      }
    });
  };

  const onStackLeave = (e: ReactMouseEvent<HTMLDivElement>) => {
    const pars = e.currentTarget.querySelectorAll<HTMLElement>('.mc-par');
    for (let i = 0; i < pars.length; i++) pars[i].style.transform = '';
  };
      ...
      >
        {TECHS.map((tech, i) => (
          ...
          >
            <div
              style={
                reducedMotion
                  ? {}
                  : {
                      animation: `mc-fl${(i % 3) + 1} ${8 + (i % 4)}s ease-in-out infinite ${(i * 0.3).toFixed(1)}s`,
                    }
              }
            >
              <div
                style={{
                  ...
                  border: '1px solid var(--border-strong)',
                  boxShadow: '0 10px 26px -16px rgba(0,0,0,.6)',
                  color: tech.brand,
                }}
              >
            ...
            <span style={{ fontSize: '.76rem', fontWeight: 600, color: 'var(--muted)' }}>
              {tech.name}
            </span>
          </div>
        ))}
    ...
    >
      <div
        ...
        style={{
          ...
          zIndex: 0,
          background:
            'radial-gradient(50% 50% at 50% 50%,rgba(34,211,238,.14),rgba(37,99,235,.08) 45%,transparent 72%)',
          filter: 'blur(10px)',
          ...(reducedMotion ? {} : { animation: 'mc-pulse 6s ease-in-out infinite' }),
        }}
      ></div>
      <TechRing
        size="clamp(300px,34vw,420px)"
        dur={46}
        rev={false}
        dotColor="rgba(34,211,238,.7)"
        reduced={reducedMotion}
      />
      <TechRing
        size="clamp(470px,54vw,660px)"
        dur={68}
        rev={true}
        dotColor="rgba(37,99,235,.7)"
        reduced={reducedMotion}
      />
      <TechParticles reduced={reducedMotion} />
      <TechLines reduced={reducedMotion} />
      {TECHS.map((tech, i) => (
        <TechNode key={tech.key} tech={tech} i={i} reduced={reducedMotion} />
      ))}
    </div>
  );
}
